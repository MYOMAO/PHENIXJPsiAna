...
void roofitB2(int Opt)
{
	...
	for(int i=0;i<NBins;i++)
	{
		...
		if(Opt==1) Cut = Form("dimuon_rapidity > 0  && Evt_Mult_FVTXN >= %d && Evt_Mult_FVTXN < %d",MultBin[i],MultBin[i+1]);
		if(Opt==2) Cut = Form("dimuon_rapidity < 0  && Evt_Mult_FVTXS >= %d && Evt_Mult_FVTXS < %d",MultBin[i],MultBin[i+1]);
		...
		ds_cut = new RooDataSet("ds","",ds, RooArgSet(*mass, *y, *Evt_Mult_FVTXN, *Evt_Mult_FVTXS), Cut.Data()); 
...
//		TCanvas* c= new TCanvas("c","",600,600);
		...
		MultRange = Form("%d <= N^{FVTX}_{TRK} <= %d",MultBin[i],MultBin[i+1]-1);
