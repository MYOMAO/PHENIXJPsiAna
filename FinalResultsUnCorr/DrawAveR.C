...
/*
	...
	for(int i = 0; i < NBins; i++){
		...
		RFinal = R * MultFactor/MUCorrFactor;
		RErrFinal = RErr * MultFactor/MUCorrFactor;

		//cout << "R = " << R << "  RFinal = " << RFinal << endl;

		RatioALICE->SetBinContent(i+1,RFinal);
		RatioALICE->SetBinError(i+1,RErrFinal);

	
		TotalALICE = TotalALICE + RFinal;
	...
	AveALICE = TotalALICE/(MultBin[NBins]- MultBin[0]);




	cout << "AveALICE = " << AveALICE << endl;
	...
	for(int i = 0; i < NBins; i++){
		...
		BinCenter = (MultBin[i] + MultBin[i+1]) * 0.5;
		MultFactor = MultiFunc->Eval(BinCenter);  //New Corr Factor


		RFinalNew = RatioALICE->GetBinContent(i+1)/AveALICE * MultFactor;
		RFinalErrNew = RatioALICE->GetBinError(i+1)/AveALICE * MultFactor;

		
		cout << "i = " << i << "    RFinalNew = " << RFinalNew << endl;
