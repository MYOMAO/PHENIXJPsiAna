...
void DrawByArms(){
	...
	for(int i = 0; i < NUse; i ++){
		...
		if(i==0){
			...
			RatioScaledCorr->GetXaxis()->SetTitle("#tilde{N}_{ch}/<#tilde{N}_{ch}>");
	...
	//   leg[0]->AddEntry(RatioScaledCorr,Form("%s %s",DataName[i].Data(),Together.Data()),"LP");
	...
	lat->DrawLatex(0.15,0.55,"#tilde{N}_{ch} and J/#psi: |#Delta y| = 0");
	lat->DrawLatex(0.15,0.49,"#tilde{N}_{ch}: 1.2 < |#eta| < 2.4, J/#psi #rightarrow #mu^{+} #mu^{-} subtracted");
	...
	for(int i = 0; i < NUse; i ++){
		...
		if(i==0){
			...
			RatioScaledCorr->SetTitle("");
			RatioScaledCorr->Draw("ep");
	//		RatioScaledCorr->GetXaxis()->SetTitle("N_{Ch}^{FVTX}/<N_{Ch}^{FVTX}>");
	//		RatioScaledCorr->GetXaxis()->SetTitle("#tilda{N}_{Ch}/<#tilda{N}_{Ch}>");
	...
	lat->DrawLatex(0.15,0.61,"pp #sqrt{s} = 200 GeV");
	lat->DrawLatex(0.15,0.55,"#tilde{N}_{ch} and J/#psi: |#Delta y| > 1");
	lat->DrawLatex(0.15,0.49,"#tilde{N}_{ch}: 1.2 < |#eta| < 2.4, J/#psi #rightarrow #mu^{+} #mu^{-} subtracted");
