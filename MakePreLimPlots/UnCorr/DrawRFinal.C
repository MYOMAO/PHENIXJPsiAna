...
*/
	if(MultOpt == 0) XName = "N_{ch}^{N}/<N_{ch}^{N}>";
	if(MultOpt == 1) XName = "N_{ch}^{S}/<N_{ch}^{S}>";
	if(MultOpt == 2) XName = "N_{ch}^{M}/<N_{ch}^{M}>";
	...
	TFile * finMU = new TFile(Form("/sphenix/user/zshi/PHENIX/JPsiAnaCodes/FinalResultsUnCorr/Infiles/FitResults_%d_%d.root",Opt, MultOpt));
	finMU->cd();
	...
	TFile * finMB = new TFile(Form("/sphenix/user/zshi/PHENIX/JPsiAnaCodes/FinalResultsUnCorr/Infiles/%sMBEvtMult.root",FileName.Data()));
	finMB->cd();
...
//	float scalefactor = 0.92/0.72;
	...
	RatioScaledCorr->GetXaxis()->SetTitleOffset(0.83);
	...
	leg->SetFillStyle(0);
	leg->SetLineWidth(3);
//	leg->AddEntry(RatioScaledCorr,"Data Points","PL");
	...
	if(MultOpt == 0) lat->DrawLatex(0.15,0.55,"N_{ch}^{N}: 1.2 < #eta < 2.4");
	if(MultOpt == 1) lat->DrawLatex(0.15,0.55,"N_{ch}^{S}: -2.4 < #eta < -1.2");
	if(MultOpt == 2) lat->DrawLatex(0.15,0.55,"N_{ch}^{M}: -1.0 < #eta < 1.0");
