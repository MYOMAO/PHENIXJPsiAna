...
RooFitResult *fit(TString variation, TString pdf, TCanvas *c, RooDataSet *ds,
		RooDataHist *dh, RooRealVar *mass, RooPlot *&outframe , int SigVar, int BackVar, int Opt) {
    ...
    RooJohnson signal2(Form("signal%d", _count), "signal", *mass, mu, lambda, gamma, delta);


    RooGaussian signal3(Form("signal%d", _count), "signal", *mass, mu, sigma);
	...
	RooPolynomial background2(Form("background%d",_count),"",*mass,RooArgSet(a0));//2nd orderpoly
	

	if(SigVar == 0 && BackVar == 0) model =	new RooAddPdf(Form("model%d", _count), "", RooArgList(signal, background),	RooArgList(nSig, nBack));
	if(SigVar == 1) model =	new RooAddPdf(Form("model%d", _count), "", RooArgList(signal2, background),	RooArgList(nSig, nBack));
	if(BackVar == 1) model = new RooAddPdf(Form("model%d", _count), "", RooArgList(signal, background2),	RooArgList(nSig, nBack));
	if(SigVar == 2) model =	new RooAddPdf(Form("model%d", _count), "", RooArgList(signal3, background),	RooArgList(nSig, nBack));
