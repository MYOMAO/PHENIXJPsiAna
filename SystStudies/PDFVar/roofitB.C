...
*/
	...
	for(int i=0;i<NBins;i++)
	{
		...
		RooFitResult* f = fit("", "", c,  ds_cut, dh, mass, frame,SigVar,BackVar);
...
//		double yield = fitYield->getVal();
		...
		c->SaveAs(Form("Plots/FitResults/%s/JPsiFit_%s_%s_%d.png",FolderName.Data(),OutName.Data(),MultOutName.Data(),_count));
		_count++;
